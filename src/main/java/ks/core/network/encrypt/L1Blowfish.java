...
public class L1Blowfish {
    private static final int PSIZE = 18;
    private static final int SSIZE = 256;

    private final UChar8 uc8 = new UChar8();
    private final ULong32 ul32 = new ULong32();

    private final int[] p = {0x243F6A88, 0x85A308D3, 0x13198A2E, 0x3707344,
    ...
    private long v = 0, w = 0;

    public L1Blowfish() {
        for (int i = 0; i < s.length; ++i) {
            for (int j = 0; j < s[i].length; ++j) {
                s[i][j] = ul32.fromLong64(s[i][j]);
            }
        }
    }

    private void init(long[] keys) {
        int i;
        int j;
        char[] c = uc8.fromArray(keys);
        ...
        v = ul32.fromInt32((int) v);
        w = ul32.fromInt32((int) w);
        ...
        for (j = 0; j < SSIZE * 2; j++, i += 2, index2 += 2) {
            ...
            s[index][index2] = ul32.fromInt32((int) v);
            s[index][index2 + 1] = ul32.fromInt32((int) w);
        }
    ...
    private void encrypt(long L, long R) {
        ...
        for (int i = 0; i < 16; i++) {
            xL ^= p[i];
            xL = ul32.fromLong64(xL);

            xR ^= f(ul32.fromLong64(xL));
            xR = ul32.fromLong64(xR);
    ...
    private int rotr(int value) {
        return (value >>> 13) | (value << (32 - 13));
    }

    public void initKeys(long[] keys) {
        init(keys);

        long rotrParam = ul32.fromLong64(keys[0] ^ 0x9C30D539);

        keys[0] = ul32.fromLong64(rotr((int) rotrParam));
