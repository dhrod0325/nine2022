...
public abstract class L1RobotAiAbstractTemplate implements L1RobotAi {
    ...
    public L1RobotAiAbstractTemplate(L1RobotInstance robot) {
        this.robot = robot;
    }
    ...
    public L1Character getAttackTarget() {
        if (hateList.getTotalHate() == 0) {
...
//
//        for (L1Character target : hateList.toTargetList()) {
//            if (target instanceof L1PcInstance) {
//                return target;
//            } else {
//                temp = target.getCurrentHp();
//
//                if (temp < min) {
//                    min = temp;
//                    minTarget = target;
...
//
//        if (minTarget == null) {
//            minTarget = hateList.getMaxHateCharacter();
//        } else {
//            return minTarget;
//        }

        return hateList.getMaxHateCharacter();
    }
    ...
    public void toAiAttack() {
        L1Character target = getAttackTarget();
    ...
    public void toSearchTarget() {
        ...
        for (L1Object obj : visibleObjects) {
            if (obj instanceof L1Character) {
                if (!L1CharPosUtils.isAttackPosition(robot, obj.getX(), obj.getY(), 10)) {
                    continue;
                }

                if (!isBow) {
                    if (!L1CharPosUtils.glanceCheck(robot, obj.getX(), obj.getY())) {
                        continue;
                    }
                }
            }
    ...
    public void toAI(long time) {
        ...
        try {
            ...
            if (aiTryCount > aiMaxTryCount && aiStatus != AI_STATUS_SETTING && aiMaxTryCount > 0) {
                if (hateList.getMaxHateCharacter() != null) {
                    blackTargetList.add(hateList.getMaxHateCharacter());
                    hateList.remove(hateList.getMaxHateCharacter());
                }
    ...
    public void toAiDead() {
        LineageAppContext.commonTaskScheduler().schedule(() -> {
            ...
            hateList.clear();
        }, Instant.now().plusMillis(RandomUtils.nextInt(3000, 5000)));
    }
