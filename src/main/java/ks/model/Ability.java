...
public class Ability {
    ...
    private int baseReduction;
    private int addedReduction;
    private int sp;

    public Ability(L1Character cha) {
        this.character = cha;
    }
    ...
    public void reset() {
        ...
        baseReduction = addedReduction = 0;
    }

    public int getEr() {
        int er = L1CalcStat.calcStatEr(getDex());

        if (character.getSkillEffectTimerSet().hasSkillEffect(AQUA_PROTECTER)) {
            er += CodeConfig.ER_AQUA;
        }

        if (character.getSkillEffectTimerSet().hasSkillEffect(DRESS_EVASION)) {
            er += CodeConfig.ER_EVASION;
        }

        if (character.getSkillEffectTimerSet().hasSkillEffect(SOLID_CARRIAGE)) {
            er += CodeConfig.ER_SOLID;
        }

        if (character.getSkillEffectTimerSet().hasSkillEffect(STRIKER_GALE)) {//게일
            er -= 99;
        }

        if (character.getSkillEffectTimerSet().hasSkillEffect(MIRROR_IMAGE)) {
            er += 20;
        }
    ...
    public int getBaseReduction() {
        return baseReduction + addedReduction;
    }

    public void addAddedReduction(int i) {
        addedReduction += i;
    }
    ...
    public boolean isNormalAbility(final int classId, final int currentLevel, final int highestLevel, final int total) {
        ...
        switch (classId) {
            case L1ClassUtils.CLASSID_PRINCE:
            case L1ClassUtils.CLASSID_PRINCESS:
                minStr = 13;
                minDex = 10;
                minCon = 10;
                minWis = 11;
                ...
                remainStats = 8;
                break;
            case L1ClassUtils.CLASSID_KNIGHT_MALE:
            case L1ClassUtils.CLASSID_KNIGHT_FEMALE:
                minStr = 16;
                minDex = 12;
                minCon = 14;
                minWis = 9;
                ...
                remainStats = 4;
                break;
            case L1ClassUtils.CLASSID_WIZARD_MALE:
            case L1ClassUtils.CLASSID_WIZARD_FEMALE:
                minStr = 8;
                minDex = 7;
                minCon = 12;
                minWis = 12;
                ...
                remainStats = 16;
                break;
            case L1ClassUtils.CLASSID_ELF_MALE:
            case L1ClassUtils.CLASSID_ELF_FEMALE:
                minStr = 11;
                minDex = 12;
                minCon = 12;
                minWis = 12;
                ...
                remainStats = 7;
                break;
            case L1ClassUtils.CLASSID_DARK_ELF_MALE:
            case L1ClassUtils.CLASSID_DARK_ELF_FEMALE:
                minStr = 12;
                minDex = 15;
                minCon = 8;
                minWis = 10;
                ...
                remainStats = 10;
                break;
            case L1ClassUtils.CLASSID_DRAGON_KNIGHT_MALE:
            case L1ClassUtils.CLASSID_DRAGON_KNIGHT_FEMALE:
                minStr = 13;
                minDex = 11;
                minCon = 14;
                minWis = 12;
                ...
                remainStats = 6;
                break;
            case L1ClassUtils.CLASSID_ILLUSIONIST_MALE:
            case L1ClassUtils.CLASSID_ILLUSIONIST_FEMALE:
                minStr = 11;
                minDex = 10;
                minCon = 12;
                minWis = 12;
    ...
    public int getSp() {
        int bonus = getBonusSp();

        int value = bonus + sp;

        return IntRange.ensure(value, 0, 90);
    }
    ...
    public int getMagicLevel() {
        if (character instanceof L1PcInstance) {
            ...
            return L1CalcStat.calcLevelMagic(pc);
        }
    ...
    public int getMagicBonus() {
        return L1CalcStat.calcStatSp(getTotalInt());
    }
