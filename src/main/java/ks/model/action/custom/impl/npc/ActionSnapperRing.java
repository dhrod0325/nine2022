...
public class ActionSnapperRing extends L1AbstractNpcAction {
    public ActionSnapperRing(String action, L1PcInstance pc, L1Object obj) {
        super(action, pc, obj);
    }

    @Override
    public void execute() {
        String act = action.replace("cc_ring_", "");

        if (act.startsWith("a")) {
            int enchant = Integer.parseInt(act.replace("a", ""));
            createChangeItem(pc, enchant, 5000047, 55000047);
        } else if (act.startsWith("b")) {
            int enchant = Integer.parseInt(act.replace("b", ""));
            createChangeItem(pc, enchant, 5000048, 55000048);
        } else if (act.startsWith("c")) {
            int enchant = Integer.parseInt(act.replace("c", ""));
            createChangeItem(pc, enchant, 5000051, 55000051);
        } else if (act.startsWith("d")) {
            int enchant = Integer.parseInt(act.replace("d", ""));
            createChangeItem(pc, enchant, 5000049, 55000049);
        }
    }
