...
import mapgen.writer.MapWriter;
import mapgen.writer.MapWriterFactory;
import mapgen.writer.TileValueWriter;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        File[] mapDirs = new File(Config.MAP_DIR).listFiles(new MapDirectoryFilter());

        if (mapDirs == null) {
            System.out.println(Config.MAP_DIR + " 이 없거나 map 디렉토리가 없습니다");
            return;
        }
        ...
        if (Config.OUTPUT_TILEVALUES) {
            TileValueWriter.writeTileValues(new File(Config.OUTPUT_DIR, "tiles.txt"), _tileList);
        }
    }
}
